...
void run_preamble ( int argc, char* argv[], ofstream& fout ) {
  ClusterSequence::print_banner(); cout << endl;
...
double deltaR_pj(PseudoJet& a, PseudoJet& b) {
  return sqrt(
    pow(a.eta() - b.eta(), 2.) + pow(correctedDiff(a.phi(), b.phi()), 2.)
  );
}
...
vector<double> calculateHigherMoments(PseudoJet& jet, double girth, bool mistake=false){
...
void find_grandma ( TClonesArray* branchGenParticles, set<int>& grandmas ) {
  ...
  for ( int i = 0; i < branchGenParticles->GetEntriesFast(); i++ ) {
    ...
    if (parti->M1 == -1 and parti->M2 == -1) {
      // for each particles without any mother, find all the granddaughters- that is the true jet
      grandmas.insert(i);
    }
  }
}
...
// but how should we even deal with the case that a particle could have been from two different grandmas?
// ideally we would want one particle to exist in only one cluster.
void find_true_clusters ( TClonesArray* branchGenParticles, vector<PseudoJet>& clusters, set<int>& grandmas ) {
  clusters.clear();
  ...
  for ( int a = 0; a < all_clusts.size(); a++ ) {
    for ( const int& element: all_clusts[a] ) {
      ...
      assert(parti != NULL);
      PseudoJet pj_content = PseudoJet(parti->Px, parti->Py, parti->Pz, parti->E);
...
int main ( int argc, char* argv[] ) {
  ofstream fout; run_preamble( argc, argv, fout );
  ...
  for ( auto& event_title: event_types ) {
    ...
    for(Long64_t entry = 0; entry < treeReader->GetEntries(); entry++){
      ...
      for ( PseudoJet& pjet: true_clusters ) {
        ...
        // TODO : add jet profile for some radius, planar flow, jet pull, dipolarity (especially color flow)
        // TODO : add jet trajectory displacement information as per the parT paper
